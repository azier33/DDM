...
class Unet3D(nn.Module):
    ...
    def forward(
        ...
        if self.has_cond:
            ...
            # Assuming cond has shape [b, n*c, 64, 64]
            if cond.shape[2:] == x.shape[2:]:
            # Concatenate along the channel dimension
                x = torch.cat((x, cond), dim=1)
                x = torch.unsqueeze(x, dim=2)
            else:
                raise ValueError(f"Shape mismatch: cond shape {cond.shape} is not compatible with x shape {x.shape}")
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_o, x_start, t, oframes, noise = None, offset_noise_strength = None):
        ...
        model_out = torch.squeeze(model_out, dim=2)
        ...
        if self.objective == 'pred_noise':
            ...
            gframes = self.decoder(x_o[:, -1:, :, :], gen_frame)
        elif self.objective == 'pred_x0':
            target = x_start
        elif self.objective == 'pred_v':
