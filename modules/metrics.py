...
class Evaluator(object):
    def __init__(self,  seq_len, value_scale, thresholds=[56, 84, 98, 112], **kwargs):
        self.metrics = {}
    ...
    def evaluate(self, true_batch, pred_batch):
        ...
        gt = self.float2int(true_batch) #经检查，gt与pred最大值均为1
        for threshold in self.thresholds:
    ...
    def done(self):
        ...
        # print_log('='*90)
        # return res_dict
        return np.mean(avg_csi), np.mean(avg_far), np.mean(avg_pod), np.mean(avg_hss), np.mean(avg_csi44), np.mean(avg_csi16), np.mean(mses), np.mean(mass), np.mean(rmses), np.mean(psnrs), np.mean(ssims), np.mean(crpss), np.mean(lpipss)
...
if __name__ == '__main__':
    ...
    targets = np.array([torch.randn(19, 128, 128).numpy(), torch.randn(19, 128, 128).numpy()]) / SCALE
    sampled = np.array([torch.randn(19, 128, 128).numpy(), torch.randn(19, 128, 128).numpy()]) / SCALE 
    mus = np.array([torch.randn(19, 128, 128).numpy(), torch.randn(19, 128, 128).numpy()]) / SCALE
    # numpy 拓展维度
    ...
    print(sampled.shape, targets.shape, mus.shape)

     
    evaluator = Evaluator(seq_len=5, value_scale=SCALE)
