...
parser.add_argument("--log_dir", type=str, default='/raid/Final_diffusion/logs/Sevir/5to5/D20250105T1945', help="Where to save logs of the model.")
parser.add_argument("--ori_log_dir", type=str, default='/raid/Final_diffusion/logs/Sevir/5to5/D20250105T1945', help="Where to save logs of the model.")
...
diffusion.load_state_dict(torch.load(os.path.join(args.log_dir,'modelshots/MM_diff_epoch50.pt'), map_location='cuda:0'))
...
diffusion_ori.load_state_dict(torch.load(os.path.join(args.ori_log_dir,'modelshots/MM_diff_epoch50.pt'), map_location='cuda:0'))
...
for step, (frames, diff_frames) in enumerate(dataloader):
    ...
    avg_p_sum += avg_pod
    avg_h_sum += avg_hss
    # evaluator_ori.evaluate(frames[:args.batch_size, args.cond_dim:args.cond_dim + args.ori_out_dim,:,:].repeat(1, 3, 1, 1), generated_images_ori.repeat(1, 3, 1, 1))
    ...
    num+=1
    if step%300 == 0:
            with open(os.path.join(args.ori_log_dir, 'test_score.txt'), 'a') as file:
                # 写入标题
                file.write("############################################################ \n************************************************************")
