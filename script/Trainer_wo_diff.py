...
from Final_diffusion.logs.MM.Diffusion_pred_no_diff import Unet3D, GaussianDiffusion
def main():
    ...
    parser.add_argument('--dist_url', default='tcp://127.0.0.1:4723', type=str, help='')
    ...
    parser.add_argument('--save_model_fre_epoch', default=50, type=int,  metavar='N', help='update model frequency')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    diffusion.load_state_dict(torch.load("/raid/Final_diffusion/logs/Sevir/5to5/D20250101T0049/modelshots/MM_diff_epoch100.pt", map_location='cuda:0'))
    ...
    for epoch in range(args.max_epoch):
        ...
        for step, (frames, diff_frames) in enumerate(dataloader):
            ...
            loss, gframes = diffusion(frames[:, 0:args.cond_dim, :, :], frames[:, (args.cond_dim):(args.cond_dim + args.out_dim), :, :], frames[:, args.cond_dim:(args.cond_dim+args.out_dim), :, :])             
            loss.backward()            
            optimizer.step()           
            lr_scheduler.step()
